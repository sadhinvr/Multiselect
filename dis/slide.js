...
(function () {
  ...
  function onchangeforall(e) {
    ...
    if (ct.type == "url") {
      ...
      let cls = ct.parentElement.nextElementSibling.firstElementChild.classList;
  ...
  function changeSlide(e) {
    ...
    if (n > 0 && n <= a.length) {
      if (n > slideNo) {
        ...
        const inpText = formHolder.querySelectorAll('input[type="text"]');
        const inpUrl = formHolder.querySelectorAll('input[type="url"]');
        const inpNum = formHolder.querySelectorAll('input[type="number"]');
        const inpTel = formHolder.querySelectorAll('input[type="tel"]');
        const select = formHolder.querySelectorAll("select:not([multiple])");
        const textarea = formHolder.querySelectorAll("textarea");
        const all = [...inpText, ...inpUrl, ...inpNum, ...inpTel, ...select, ...textarea];
        let alltrue = true;

        // const allR = a.querySelector('[req]')
        all.forEach(cur => {
          if (cur.classList.contains("mul_inp_hidden")) {
            let cls = cur.parentElement.nextElementSibling.firstElementChild.classList;
