...
function multiSelect(q, i) {
  ...
  function selectFun(select) {
    ...
    function boxPosition() {
      ...
      dropDownBox.style.top = `${multiSelectionRect.y + window.scrollY - dropDownBox.getBoundingClientRect().height}px`;
      if (dropDownBox.getBoundingClientRect().bottom + dropDownBox.getBoundingClientRect().height >= window.innerHeight) {
        dropboxBottom = false;
      ...
      if (dropboxBottom) {
        dropDownBox.style.top = `${multiSelectionRect.y + multiSelectionRect.height + window.scrollY}px`;
      } else {
        dropDownBox.style.top = `${multiSelectionRect.y + window.scrollY - dropDownBox.getBoundingClientRect().height}px`;
      }
      dropDownBox.style.left = `${multiSelectionRect.x}px`;
      dropDownBox.style.right = `${multiSelectionRect.right - multiSelectionRect.width}px`;
    }
    boxPosition();
    function resize() {
      multiSelectionRect = multiSelectOptions.getBoundingClientRect();
